...
/*   Updated: 2024/06/22 19:35:31 by hrochd           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	interpolate(int start, int end, int steps, int return_index)
{
	...
	while (i < steps)
	{
		...
		result[i] = (result_color.red << 16)
			| (result_color.green << 8)
			| result_color.blue;
		i++;
	}
...
void	change_palette(t_fractal *fractal)
{
	...
	if (fractal->palette_index % 11 == 0)
		set_palette(0xffd900, 0x191971, 0xffd900, fractal);
	else if (fractal->palette_index % 11 == 1)
		set_palette(BLACK, 0x2f3293, 0x19ffff, fractal);
	else if (fractal->palette_index % 11 == 2)
		set_palette(0x141B41, 0xffffff, 0x306cab, fractal);
	else if (fractal->palette_index % 11 == 3)
		set_palette(BLACK, 0x001801, 0x00ff2a, fractal);
	else if (fractal->palette_index % 11 == 4)
		set_palette(0x1E212B, 0x4e8c31, 0xffc800, fractal);
	else if (fractal->palette_index % 11 == 5)
		set_palette(WHITE, 0x001801, 0x00ff2a, fractal);
	else if (fractal->palette_index % 11 == 6)
		set_palette(0x141B41, 0x306cab, 0xffffff, fractal);
	else if (fractal->palette_index % 11 == 7)
		set_palette(BLACK, 0x00ff2a, 0x001801, fractal);
	else if (fractal->palette_index % 11 == 8)
		set_palette(0x00FFFF, 0x1d2672, 0xc33763, fractal);
	else if (fractal->palette_index % 11 == 9)
		set_palette(BLACK, 0x023140, 0x38aecc, fractal);
	else if (fractal->palette_index % 11 == 10)
